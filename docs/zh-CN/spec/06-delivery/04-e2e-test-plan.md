...
- **步骤**：选中单条消息中的文字并点击浮动按钮；重复验证助手正文、多行选区、跨消息选区、工具输出、清除选区、滚动和切换会话。
- **预期**：单条消息正文的选区旁直接出现“添加到对话”，无需打开右键菜单。仅所选文字作为 Markdown 引用追加到当前草稿，保留原文和文件引用、聚焦末尾，不自动发送。其他选区不显示按钮；清除选区、滚动、调整窗口或切换面板后关闭，跨会话不插入。右键菜单仍可引用整条消息。
- **链接规格**：`04-ux/08-component-spec.md` §8.3 / §8.5、`04-ux/09-interaction-patterns.md` §1.5、ADR 0306
- **验收**：C（聊天流）、质量
- **里程碑**：M5
- **状态**：Chromium 组件验证（`node scripts/e2e-message-edit-copy.mjs`）和草稿追加验证（`pnpm test:e2e:composer-paste`）已覆盖；完整应用视觉场景仍为草稿。
