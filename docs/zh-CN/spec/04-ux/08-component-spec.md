...
- 悬停操作：气泡下安静的仅图标操作芯片 — 空闲助手回合显示复制；
  ...
  在单条用户消息或助手正文内选中文字时，选区旁直接显示“添加到会话”浮动按钮，无需打开右键菜单（ADR 0306）。所选文字成为当前会话输入框上方的数量卡片（如“1 条注释”），点击可预览并逐条移除；可编辑正文、文件引用和图片附件保持不变。发送时再把所选原文序列化为引用上下文，发送失败后恢复卡片；该操作本身不自动发送。工具输出、思考内容、输入框及跨消息选区不显示按钮；清除选区、滚动、调整窗口或切换面板时关闭。右键菜单保留整条消息的回退入口。
