...
- 悬停操作：气泡下安静的仅图标操作芯片 — 空闲助手回合显示复制；
  ...
  在单条用户消息或助手正文内选中文字时，选区旁还会直接显示“添加到对话”浮动按钮，无需打开右键菜单（ADR 0306）。它只把所选文字作为 Markdown 引用追加到当前会话草稿，保留已有文字和文件引用，不自动发送。工具输出、思考内容、输入框及跨消息选区不显示按钮；清除选区、滚动、调整窗口或切换面板时关闭。右键菜单保留整条消息的回退入口。
