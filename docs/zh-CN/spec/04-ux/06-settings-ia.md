...
- OpenCode 会话发现会读取绝对路径 `XDG_DATA_HOME/opencode` 下的
  `opencode.db`；XDG 未设置或为相对路径时，则读取
  `~/.local/share/opencode`。同时继续支持旧版 `storage/session`、`message`
  和 `part` JSON 布局。同一会话同时存在于两种存储时只显示一次，并以数据库
  副本为准；空会话不提供导入。
- SQLite 作为外部只读数据源处理，不执行迁移、写入、恢复或 checkpoint。
  读取事务包含 WAL 中已提交的数据，并将锁等待限制为 250 ms。数据库缺失、
  被锁定、损坏或不兼容时，旧版 JSON 扫描仍可继续。选中会话转换失败时，
  会在宿主导入前整体失败，不会静默保存不完整的记录。
- 支持的 SQLite 结构为 OpenCode v1 的 `session`、`message` 和 `part` 表，
  已按 v1.18.26（`774cc7c`）核对。会话和消息使用完整的键集分页读取；消息按
  `(time_created, id)` 排序，部件按 `id` 排序。来源 ID、时间戳、项目目录和
  助手模型元数据沿用现有导入语义。可见的非合成文本，以及工具输入、输出和
  错误仍由现有转换器处理；推理、附件和结构性部件不在内容导入范围内。
  独立的 v2 `session_message` 投影暂不导入。
