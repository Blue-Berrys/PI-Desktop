...
- 在单条用户消息或助手正文中选中文字，选区旁直接显示“添加到对话”。点击后仅将
  所选内容作为 Markdown 引用追加到当前会话草稿，保留原草稿和文件引用，不自动
  发送。清除选区、滚动、调整窗口或切换面板会关闭按钮；工具输出、思考内容和
  跨消息选区不显示按钮。右键菜单仍提供整条消息的引用入口（ADR 0306）。
